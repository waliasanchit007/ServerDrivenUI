...
- ⏳ **Batch 3.4 (in progress)** — PullToRefresh: PullToRefreshBox @ ID 150. Wraps M3's `PullToRefreshBox`. Guest holds `isRefreshing` Boolean, flips true on `onRefresh`, kicks off async work, flips false when done. Showcase uses `LaunchedEffect(refreshCount) { delay(1200) }` to simulate async. Awaiting on-device verification.
