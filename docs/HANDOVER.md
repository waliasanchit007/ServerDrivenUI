...
- ⏳ **Batch 3.3 (in progress)** — Pagers: HorizontalPager / VerticalPager / PagerIndicator @ IDs 140–142. First "indexed-children" pattern (host derives pageCount from `widgets.size`, renders only `widgets[i]` for page i). PagerIndicator is host-rolled (no 1:1 M3) — Row of dots with the active dot slightly larger as a non-color cue. Awaiting on-device verification.
