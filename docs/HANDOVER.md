...
- ⏳ **Batch 3.5 (in progress)** — Large-screen nav: NavigationRail / NavigationRailItem / ModalNavigationDrawer / NavigationDrawerItem @ IDs 160–163. First controlled-component drawer state via guest-held `drawerOpen` Boolean, host-side LaunchedEffect sync to M3's DrawerState, snapshotFlow report-back of user gestures. ModalNavigationDrawer needs its own top-level surface so the showcase pushes a dedicated `NavDrawerDemoScreen` instead of embedding inline. Awaiting on-device verification.
