...
| 2026-05 | **Tier 3 modifier additions** (tags 12–17): Border (12, rectangular only in v1), Clip (13, rounded-rect cornerRadiusDp), ClipCircle (14, full circle), WrapContentWidth (15), WrapContentHeight (16), AspectRatio (17, Double ratio). Brings layout-modifier total to 16. Tag 7 still unused (was reserved for the Clickable modifier that didn't ship — see gotcha #8). Verified visually on iOS sim: rounded card + circular avatar + 16:9 frame + wrapped tag pill all render correctly. | claude |
| 2026-05 | Border defers color resolution like Background: `SchemaColor.toComposeColor()` is @Composable and can't run inside `forEachUnscoped`'s non-composable lambda. Fix follows the existing Background pattern — store thicknessDp + SchemaColor during iteration, apply `m.border(thickness.dp, color.toComposeColor())` after the loop. Latest Border in the chain wins (matches Background semantics). Documented in KDoc. | claude |
| 2026-05 | Border in v1 is always rectangular: rounded-stroke shapes need an additive `cornerRadiusDp` parameter on Border (or pass-through to Compose's `border(width, color, shape)`). Today the rectangular stroke renders around any clip-rounded shape, so corner pixels get clipped away — visible in the showcase as the small "Clip(16) + Border" demo. Workaround: use Card / OutlinedCard widgets when you need rounded stroke + container together. | claude |
| 2026-05 | **Snackbar host queue** — `HostSnackbar` Zipline service replaces the inline `Snackbar` widget for the recommended pattern. Architecture: singleton `SnackbarHub.state: SnackbarHostState` survives recompositions; `RealHostSnackbar.show()` runs `scope.launch { state.showSnackbar(...) }` (M3 internally serializes via Mutex → natural FIFO queue, no extra state needed); `App.kt` renders `SnackbarHost(SnackbarHub.state)` at bottom-center. Guest exposes top-level `showHostSnackbar(message, actionLabel?, durationMillis)` backed by `HostSnackbarBridge.instance`. The original Tier 2 `Snackbar` widget is kept as a static visual demo. | claude |
| 2026-05 | Snackbar `RealHostSnackbar.scope` is `lazy` not eager: Dispatchers.Main on iOS Kotlin/Native at bindServices time may not yet be wired (bindServices runs before Compose UI mounts and can run before the main coroutine dispatcher is fully available). Lazy defers resolution until the first `show()` call when the dispatcher is reliably available. Both `RealHostSnackbar.show()` (host) and `showHostSnackbar()` (guest) wrap the body in try/catch + println — observed empirically that an uncaught throw out of a Zipline RPC tears down the surrounding UI (blank screen on iOS). Silent-fail-with-log keeps the rest of the app alive. | claude |
...
| 2026-05 | **Snackbar action-result callback** shipped as additive `showWithResult(message, actionLabel, durationMillis, onResult)` method on `HostSnackbar` (alongside existing `show()`). Rationale for new method vs adding `onResult` parameter to `show()`: ZiplineService method signatures are wire format. Adding a parameter would break old guests calling new hosts and vice versa; adding a new method is additive. Guest helper `showHostSnackbar()` accepts optional `onResult` and routes to the cheaper fire-and-forget `show()` when null, `showWithResult()` otherwise. Result is mapped from M3's SnackbarResult: `ActionPerformed` → true, `Dismissed` → false. Inner try/catch around `onResult.invoke(...)` on the host side so a guest lambda that's been disposed (screen unmounted) doesn't kill the coroutine scope. | claude |
| 2026-05 | **Rounded Border modifier** shipped as additive `cornerRadiusDp: Int = 0` field on `Border` (tag 12). Konduit's `@Modifier` codegen + kotlinx.serialization accept Kotlin default values — verified end-to-end. Backward compatibility: old payloads without the new field decode to 0 = the previous rectangular behavior. Host uses `RoundedCornerShape(cornerRadiusDp.dp)` unconditionally; `RoundedCornerShape(0.dp)` is equivalent to `RectangleShape`, so no branching needed in the apply path. This was the first `@Modifier` with a default value; the pattern now becomes the canonical way to add fields to existing modifiers without breaking wire. | claude |
